...
from .types import AudioInput, DeviceType, DType
...
class SamAudioSession:
    ...
    def cache_info(self) -> dict[str, Any]:
        from .download import get_cache_dir, list_cached_models
        path = Path(self.cache_dir) if self.cache_dir is not None else get_cache_dir()
        return {"path": str(path), "exists": path.exists(), "model": self._checkpoint.get("model_id"),
                "checkpoints": list_cached_models(path)}
